...
import { ICategoryDetail, ICreateCategory, IMenuDetail, ISubDetail, IUpdateCategory } from '../model/category';
...
const initialState = {
  ...
  menus: [] as ReadonlyArray<IMenuDetail>,
  menu: {} as ISubDetail,
};
...
export const getMenu = createAsyncThunk(
  'client/get-menu',
  async () => {
    return await axios.get<any>(`${URL_PATH.API.GET_MENU}`);
  },
  {
    serializeError: serializeAxiosError,
  }
...
export const CategorySlice = createSlice({
  ...
  extraReducers(builder) {
    builder
      ...
      .addMatcher(isFulfilled(getMenu), (state, action) => {
        state.loading = false;
        state.menus = action.payload.data?.menu;
      })
      .addMatcher(isPending(getMenu), (state, action) => {
        state.loading = true;
      })
      .addMatcher(isRejected(getMenu), (state, action) => {
        state.loading = false;
        const httpStatusCode = action.error['response']?.status;
      });
  },
